...
class concurrent_http_connection_impl 
...
public:
    ...
    explicit concurrent_http_connection_impl(io_service* ios) 
        : m_io_service(ios)
        , m_options(new http_connection_options_impl)
        , m_port(0)
    {
        XUL_LOGGER_INIT("concurrent_http_connection");
        XUL_DEBUG("new " << m_options->receive_buffer_size);
        set_listener(NULL);
        init_client();
    }
    virtual ~concurrent_http_connection_impl()
    {
        XUL_DEBUG("delete");
        this->destroy();
    }
    ...
    virtual bool send_data(const uint8_t* data, int size)
    {
        if (!is_started())
            return false;
        if (!m_requests.empty() || !m_requests.front()->header_send)
        {
            assert(false);
            return false;
        }
    ...
    virtual void close()
    {
        XUL_DEBUG("close " << is_started() << xul::make_tuple(m_requests.size(), m_socket, m_socket->is_connected()));
        m_requests.clear();
        do_close();
    }
    virtual void do_close()
    {
        XUL_DEBUG("do_close " << is_started() << xul::make_tuple(m_requests.size(), m_socket, m_socket->is_connected()));
        m_state.reset();
        m_socket->destroy();
        if (m_name_resolver)
        {
    ...
    void handle_error(int errcode)
    {
        ...
        http_connection_request_info_ptr reqInfo = m_requests.front();
        m_requests.pop_front();
        XUL_FOREACH_CONST(std::list<http_connection_request_info_ptr>, m_requests, iter)
        {
            http_connection_request_info* item = (*iter).get();
            item->reset_handling_state();
        }
        if (!m_requests.empty())
        {
            connect_host(m_requests.front().get());
        }
    ...
    virtual void on_socket_connect(tcp_socket* sender)
    {
        ...
        XUL_FOREACH_CONST(std::list<http_connection_request_info_ptr>, m_requests, iter)
        {
            http_connection_request_info* reqInfo = (*iter).get();
    ...
    {
        XUL_DEBUG("on_complete " << xul::make_tuple(m_requests.size(), total_size, size));
        if (m_requests.empty())
        {
            //assert(false);
            abort_handling();
        ...
        if (size > 0)
        {
            ...
            if (!reqInfo->process(data, size))
            {
                XUL_ERROR("on_complete process abort");
                return false;
            }
        }
        return true;
    ...
    {
        XUL_DEBUG("on_header " << xul::make_tuple(m_requests.size(), resp->get_status_code()));
        if (m_requests.empty())
        {
            //assert(false);
            abort_handling();
        ...
        if (!m_listener->on_http_response(this, resp, m_requests.front()->url->get_original_string()))
            return false;
        return true;
    }
    bool on_content(url_request* req, url_response* resp, const uint8_t* data, int size)
    {
        XUL_DEBUG("on_content " << xul::make_tuple(m_requests.size(), size));
        if (m_requests.empty())
        {
            //assert(false);
            abort_handling();
    ...
    {
        XUL_ERROR("on_error " << xul::make_tuple(m_requests.size(), errcode));
        abort_handling();
...
protected:
    void check_receive()
    {
        if (!m_state.pause_receive && m_socket->is_connected() && !m_socket->is_receiving())
        {
            m_socket->receive(m_options->receive_buffer_size);
        }
    }
    ...
    {
        XUL_DEBUG("redirect " << reqInfo->url->get_original_string());
        do_close();
        do_request(reqInfo);
        return true;
    }
    ...
    {
        XUL_WARN("abort_handling " << xul::make_tuple(m_requests.size(), m_socket->is_connected()));
        do_close();
    }
    bool do_request(http_connection_request_info* reqInfo)
    {
        XUL_DEBUG("do_request " << xul::make_tuple(m_requests.size(), m_socket->is_connected()) 
            << " " << m_host << ":" << m_port 
            << " " << reqInfo->request->get_header("Range", "") 
            << " " << reqInfo->url->get_original_string());
        if (!m_requests.empty() && !m_requests.back()->header_send)
        {
            XUL_DEBUG("queue unsend request " << xul::make_tuple(m_requests.size(), m_host, m_port));
            m_requests.push_back(reqInfo);
            return true;
        }
        if (m_socket->is_connected() && is_same_host(reqInfo))
        {
            XUL_DEBUG("use existing connection " << xul::make_tuple(m_requests.size(), m_host, m_port));
            m_requests.push_back(reqInfo);
            write_request(reqInfo);
            return true;
        }
        if (m_requests.empty())
        {
            XUL_DEBUG("use new connection " << xul::make_tuple(m_requests.size(), m_host, m_port));
            m_requests.push_back(reqInfo);
    ...
    void connect_host(http_connection_request_info* reqInfo)
    {
        ...
        do_close();
        m_host = reqInfo->url->get_host();
        m_port = reqInfo->url->get_port();
...
private:
    ...
    std::list<http_connection_request_info_ptr> m_requests;
};
